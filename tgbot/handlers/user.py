...
async def in_start(message: Message, bot: Bot, state: FSMContext):
    ...
    if message.text == "Завершити діалог":
        ...
        if oper_id and user_id:
            ...
            await message.bot.send_message(oper_id, text="Сесію завершено.",reply_markup=start_keyboard)
            await message.bot.send_message(user_id, text="Сесію завершено.", reply_markup=start_keyboard)
            delete_session(session)

    if str(message.from_user.id) in get_admins() and message.text != "Завершити діалог":
        await bot.send_message(get_user_id_by_session(session), message.text)
    else:
        if message.text != "Завершити діалог":
            await bot.send_message(get_operator_id_by_session(session), message.text)
